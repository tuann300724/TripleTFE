...
export default function CourtDetailPage() {
    ...
    if (!branch) {
        return (
            <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 bg-[#0c1219] text-white">
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        // Giả lập gửi API đặt sân thành công
    ...
    return (
        <div className="bg-[#08101a] min-h-screen text-slate-100 pb-20">
            {/* ══ BANNER HERO ══ */}
            <div className="relative w-full h-[280px] overflow-hidden group">
                ...
                {/* back button */}
                <Link to="/booking" className="absolute top-[18px] left-[18px] z-10 flex items-center gap-1.5 bg-black/45 hover:bg-black/65 backdrop-blur-md text-white px-3.5 py-1.5 rounded-full text-xs font-bold transition-all border border-white/10 shadow-lg active:scale-95">
            ...
            <div className="max-w-[1000px] mx-auto px-4 -mt-20 relative z-20">
                ...
                <div className="mb-6 animate-fade-in-up">
                    <h1 className="text-white text-2xl md:text-3xl font-extrabold leading-snug drop-shadow-md">
                        {branch.name}
                    </h1>
                    <p className="mt-2 text-slate-400 text-xs md:text-sm flex items-center gap-1.5 leading-relaxed">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
                    {[
                        { icon: <Trophy size={14} className="text-amber-400" />, text: `${branch.rating} ★`, bg: "bg-amber-500/10 border-amber-500/20 text-amber-200" },
                        { icon: <Clock size={14} className="text-blue-400" />, text: branch.openTime, bg: "bg-blue-500/10 border-blue-500/20 text-blue-200" },
                        { icon: <Sparkles size={14} className="text-emerald-400" />, text: branch.priceRange, bg: "bg-emerald-500/10 border-emerald-500/20 text-emerald-200" },
                        { icon: <Phone size={14} className="text-purple-400" />, text: branch.phone, bg: "bg-purple-500/10 border-purple-500/20 text-purple-200" },
                    ].map((b, i) => (
                    ...
                    <div className="col-span-2 md:col-span-1 flex items-center justify-center gap-2 border bg-emerald-500/15 border-emerald-500/30 rounded-xl py-2 px-3 text-[#00f5a0] text-xs font-black shadow-sm">
                ...
                {/* ── THANH NAV TABS ── */}
                <div className="flex gap-1 bg-white/5 border border-white/5 rounded-2xl p-1.5 mb-8 w-fit overflow-x-auto">
                    {[
                    ...
                    <div className="animate-[fadeUp_0.3s_ease_both]">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                            <div className="flex items-center gap-5">
                                {[
                                    { color: "bg-white/5 border-white/15", label: "Trống" },
                                    { color: "bg-rose-500/70 border-0", label: "Đã đặt" },
                                    { color: "bg-emerald-500 shadow-inner", label: "Đang chọn" }
                                ].map((l, idx) => (
                            ...
                            <div className="flex items-center gap-2">
                                <label className="text-[11px] text-slate-400 font-extrabold uppercase tracking-wide">Chọn ngày thi đấu:</label>
                                <input
                                    type="date"
                                    value={selectedDate}
                        ...
                        {/* Lưới chọn slot giờ */}
                        <div className="border border-white/5 rounded-2xl overflow-hidden bg-[#0b1422] shadow-xl">
                            <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                                <table className="border-collapse table-fixed min-w-max w-full">
                                    <thead>
                                        <tr>
                                            <th className="sticky left-0 z-20 w-[140px] bg-[#0d1627] border-r border-white/10 border-b border-white/5 p-3 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                                Sân / Giờ
                                            </th>
                                            {timeSlots.map(t => (
                                                <th key={t} className="w-11 bg-[#0c1525] border-r border-white/5 border-b border-white/5 py-2.5 text-center text-[9px] font-black text-slate-500">
                                                    {t}
                                                </th>
                                            ))}
                                        </tr>
                                    ...
                                    <tbody>
                                        {branch.courts.map((court, ri) => (
                                            <tr key={court.id} className={ri % 2 === 0 ? "bg-[#0b1422]" : "bg-[#0c1525]"}>
                                                ...
                                                <td className={`sticky left-0 z-10 w-[140px] border-r border-white/10 border-b border-white/5 px-3 py-2.5 ${ri % 2 === 0 ? "bg-[#0d1730]" : "bg-[#0d1830]"} shadow-[2px_0_5px_rgba(0,0,0,0.3)]`}>
                                                    <p className="m-0 text-xs font-black text-emerald-400 leading-none">
                                                        {court.name.split(" - ")[0]}
                                                    </p>
                                                    <p className="m-0 mt-1 text-[9px] text-slate-400 font-bold">{court.price}</p>
                                                </td>
                                                {timeSlots.map(time => {
                                                    ...
                                                    if (booked) cellClass = "bg-rose-500/70 cursor-not-allowed";
                                                    else if (chosen) cellClass = "bg-emerald-500 shadow-[inset_0_0_8px_rgba(0,0,0,0.2)]";
                        ...
                        <div className={`mt-6 border rounded-2xl p-5 flex flex-col md:flex-row items-center justify-between gap-5 transition-all duration-300 ${
                            ...
                            <div className="flex gap-8 text-center md:text-left">
                                ...
                                <div className="border-l border-white/10 px-8">
                                    ...
                                    <p className="m-0 mt-1.5 text-2xl font-black text-amber-400">
                                ...
                                {selectedCells.length > 0 && (
                                    <div className="border-l border-white/10 pl-8 hidden sm:block">
                                        ...
                                        <p className="m-0 mt-1.5 text-2xl font-black text-white">{selectedCells.length} ô</p>
                                    </div>
                                )}
                            </div>
                            <button
                ...
                {activeTab === "form" && (
                    <div className="animate-[fadeUp_0.3s_ease_both] grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <form onSubmit={handleFormSubmit} className="lg:col-span-2 bg-[#0d1627] border border-white/5 rounded-2xl p-6 flex flex-col gap-5">
                            <h2 className="text-white text-base font-black flex items-center gap-2 mb-2 border-b border-white/5 pb-3">
                                ...
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full bg-[#08101a] border border-white/10 rounded-xl py-3 pl-9 pr-3 text-xs text-white placeholder-slate-500 outline-none focus:border-emerald-500 transition-colors font-semibold"
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wide">Địa chỉ Email</label>
                                <div className="relative">
                                    <input
                                        type="email"
                                        value={customerEmail}
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wide">Ghi chú thêm</label>
                                <div className="relative">
                                    <textarea
                                        value={customerNotes}
                                        onChange={e => setCustomerNotes(e.target.value)}
                                        rows={3}
                                        className="w-full bg-[#08101a] border border-white/10 rounded-xl py-2.5 pl-9 pr-3 text-xs text-white placeholder-slate-500 outline-none focus:border-emerald-500 transition-colors font-semibold resize-none"
                                        ...
                                        <label
                                            ...
                                            className={`border rounded-xl p-3 flex flex-col gap-1 cursor-pointer transition-all ${
                                                paymentMethod === method.id
                                                    ...
                                                    : "bg-[#08101a] border-white/5 text-slate-400 hover:border-white/15"
                                            }`}
                                        >
                                            <input
                                                type="radio"
                                            ...
                                            <span className="text-xs font-black flex items-center gap-1">
                                                <CreditCard size={12} className={paymentMethod === method.id ? "text-emerald-400" : "text-slate-500"} />
                                                {method.title}
                                            </span>
                                            <span className="text-[9px] text-slate-500 font-semibold leading-tight">{method.desc}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>

                            <div className="flex justify-between items-center mt-3 pt-4 border-t border-white/5 gap-3">
                                <button
                                    type="button"
                                    onClick={() => setActiveTab("booking")}
                                    className="flex items-center gap-1 bg-white/5 border border-white/10 hover:bg-white/10 text-white font-bold text-xs px-5 py-3 rounded-xl transition-all cursor-pointer"
                                >
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 px-8 py-3.5 rounded-xl text-xs font-black transition-all shadow-lg ${
                                        selectedCells.length
                                            ? "bg-emerald-500 hover:bg-emerald-400 text-[#0c1219] hover:shadow-emerald-500/20 hover:-translate-y-0.5 cursor-pointer active:translate-y-0"
                                            : "bg-slate-800 text-slate-500 cursor-not-allowed"
                                    }`}
                                >
                        ...
                        <div className="bg-[#0d1627] border border-white/5 rounded-2xl p-5 flex flex-col gap-4 h-fit">
                            <h3 className="text-white text-xs font-black uppercase tracking-wider border-b border-white/5 pb-2.5">
                                Tóm tắt đặt sân
                            </h3>
                            <div className="flex flex-col gap-3">
                                <div className="flex justify-between items-start text-xs gap-3">
                                    <span className="text-slate-400 font-bold shrink-0">Chi nhánh:</span>
                                    <span className="text-white font-extrabold text-right leading-relaxed">{branch.name}</span>
                                </div>
                                <div className="flex justify-between items-center text-xs gap-3">
                                    <span className="text-slate-400 font-bold">Ngày chơi:</span>
                                    <span className="text-emerald-400 font-extrabold">{selectedDate}</span>
                                </div>
                                <div className="border-t border-dashed border-white/10 pt-3 flex flex-col gap-2">
                                    <span className="text-slate-400 font-bold text-xs">Các khung giờ chọn ({selectedCells.length}):</span>
                                    {selectedCells.length === 0 ? (
                                        <p className="text-[10px] text-amber-500 font-bold">Chưa chọn khung giờ nào. Vui lòng quay lại tab Chọn giờ.</p>
                                    ) : (
                                        <div className="max-h-[140px] overflow-y-auto flex flex-col gap-1.5 pr-1">
                                            {selectedCells.map((cell, idx) => {
                                                const court = branch.courts.find(c => c.id === cell.courtId);
                                                return (
                                                    <div key={idx} className="flex justify-between items-center bg-[#08101a] border border-white/5 rounded-lg px-2.5 py-1.5 text-[10px] font-bold">
                                                        <span className="text-slate-300">{court?.name.split(" - ")[0]}</span>
                                                        <span className="text-emerald-400 flex items-center gap-1">
                                                            <Clock size={9} />
                                                            {cell.time}
                                                        </span>
                                                        <span className="text-amber-400">{(court?.pricePerSlot || 0).toLocaleString("vi-VN")}đ</span>
                                                    </div>
                                                );
                                            })}
                                        </div>
                                ...
                                <div className="border-t border-white/5 pt-3 mt-1 flex justify-between items-baseline">
                                    <span className="text-slate-400 font-bold text-xs">Tổng cộng:</span>
                                    <span className="text-amber-400 font-black text-xl">{totalMoney.toLocaleString("vi-VN")}đ</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            {(branch.gallery || []).map((imgUrl, idx) => (
                                <div
                                    ...
                                    className="relative rounded-2xl overflow-hidden aspect-video bg-[#0d1627] border border-white/5 cursor-zoom-in group shadow-lg hover:border-emerald-500/40 transition-all duration-300"
                                >
                                    <img
                                        src={imgUrl}
                ...
                {activeTab === "info" && (
                    <div className="animate-[fadeUp_0.3s_ease_both] grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Giới thiệu */}
                        <div className="bg-[#0d1627] rounded-2xl p-6 border border-white/5 shadow-xl flex flex-col gap-3">
                            <h2 className="text-white text-sm font-black uppercase tracking-wider flex items-center gap-2 border-b border-white/5 pb-2.5">
                                <Layers size={14} className="text-emerald-400" /> Giới thiệu chi nhánh
                            </h2>
                            <p className="text-slate-400 text-xs leading-relaxed leading-7 m-0">
                                {branch.description}
                            </p>
                        </div>
                        ...
                        <div className="bg-[#0d1627] rounded-2xl p-6 border border-white/5 shadow-xl flex flex-col gap-3">
                            <h2 className="text-white text-sm font-black uppercase tracking-wider flex items-center gap-2 border-b border-white/5 pb-2.5">
                                <CheckCircle2 size={14} className="text-emerald-400" /> Tiện ích có sẵn
                            </h2>
                            <div className="grid grid-cols-2 gap-2">
                                {(branch.amenities || []).map((a, i) => (
                                    <div key={i} className="flex items-center gap-2 bg-[#08101a] border border-white/5 rounded-xl py-2 px-3 text-emerald-300 text-[10px] font-bold shadow-sm">
                                        <span className="text-emerald-400 shrink-0">{amenityIcon(a)}</span>
                        ...
                        {/* Sân con */}
                        <div className="md:col-span-2 bg-[#0d1627] rounded-2xl p-6 border border-white/5 shadow-xl flex flex-col gap-4">
                            <h2 className="text-white text-sm font-black uppercase tracking-wider flex items-center gap-2 border-b border-white/5 pb-2.5">
                                <Trophy size={14} className="text-amber-400" /> Danh sách sân thi đấu
                            </h2>
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                                {branch.courts.map(c => (
                                    <div key={c.id} className="bg-[#08101a] border border-white/5 hover:border-emerald-500/20 rounded-xl p-3.5 transition-all shadow-sm">
                                        <p className="m-0 text-xs font-black text-white">{c.name}</p>
                                        <p className="m-0 mt-1 text-[10px] text-slate-500 font-bold">{c.type}</p>
                                        <p className="m-0 mt-2 text-[#00f5a0] text-sm font-black">{c.price}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
